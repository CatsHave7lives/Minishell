...
/*   Updated: 2025/06/17 06:43:53 by aessaber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_env	*env_set_node(t_env **env, char *target_var, char *new_value, t_gc **gc)
{
	...
	if (env_node)
	{
		...
		env_node->value = msh_strdup(new_value, gc);
		if (!env_node->value)
		{
	...
	if (!env_node)
		return (NULL);
	return (env_set_node_all(env_node, target_var, new_value));
}

int	env_set_node_value(t_env **env, char *target_var, char *new_value)
{
	t_env	*env_node;

	if (!env || !target_var || !new_value)
		return (EXIT_FAILURE);
	env_node = env_get_node(env, target_var);
	if (env_node)
	{
	...
	else
	{
		...
		if (env_set_node_all(env_node, target_var, new_value) == EXIT_FAILURE)
		{
